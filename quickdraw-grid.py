...
# Check if the user wants to run the script again after completion
while play_again:

    # Collect user's choice of category (repeat until valid category is selected)
    print('\nWhat is your "Quick,Draw!" category of choice?')
    while True:
        qd = QuickDrawData()
    ...
    print(f"\nYour file '{selected_category}{file_index}.svg' can be found here: {os.getcwd()}")
    play = input("\nWould you like to generate another drawing? [Y/N]: ")
    play_answers = ["y", "yes", "Yes", "n", "no", "No"]
    while play not in play_answers:
        play = input("Please enter a valid answer [Yes or No]: ")
    if play == "n" or play == "no" or play == "No":
        print("Okay, no worries. Bye!")
        play_again = False